        ...
        try:
            ...
            if settings["method"] == "Button":
                ...
                if settings["url"] and settings["api"]:
                    ...
                    # Add web_link for page mode
                    page_mode = await u_db.get_chl_page_mode(msg.chat.id)
                    if page_mode:
                        # Get channel page code for privacy
                        page_code = await u_db.get_chl_page_code(msg.chat.id)
                        if not page_code:
                            # Generate new page code if not exists
                            page_code = await u_db.regenerate_chl_page_code(msg.chat.id)
                        web_link = f"{Server.URL}/sl/{str(log_msg.id)}/{page_code}?channel={msg.chat.id}"
                        basic_format_dict["web_link"] = web_link
                    else:
                        basic_format_dict["web_link"] = basic_format_dict["stream_link"]
