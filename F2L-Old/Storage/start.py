...
async def start(client, message):
    ...
    # Handle both formats: /start and /start 3869
    if usr_cmd == "/start" or message.text.strip() == "/start":
        ...
        # Extract message ID properly from different start formats
        if " " in message.text:
            # Handle "/start 3869" or "/start download_3869" format
            cmd_part = message.text.split()[-1]
            if cmd_part.startswith("download_"):
                usr_cmd = cmd_part.replace("download_", "")
            else:
                usr_cmd = cmd_part
        else:
            # Handle "/start_3869" or "/start_download_3869" format  
            cmd_part = message.text.split("_")[-1]
            if cmd_part.isdigit():
                usr_cmd = cmd_part
            else:
                # Handle case where command is like "start_download_3869"
        ...
        # Try to convert to int, if fails it means invalid format
        try:
            message_id = int(usr_cmd)
        except ValueError:
            await message.reply_text("❌ Invalid command format. Please use a valid file link.")
            return
        ...
        if Telegram.AUTH_CHANNEL2 and not await is_subscribed(client, message, Telegram.AUTH_CHANNEL2):
            ...
            btn = [[InlineKeyboardButton("🔰 𝙲𝙷𝙰𝙽𝙽𝙴𝙻 1 🔰", url= Telegram.AUTH_GROUP2),
                ...
                [
                            ...
                            "🔄 Refresh / Try Again",
                            url=f"https://t.me/{(await client.get_me()).username}?start={message_id}",
                        )
                ],
            ]
            try:
        ...
        get_msg = await client.get_messages(
            chat_id=Telegram.FLOG_CHANNEL, message_ids=message_id
        )
